...
// Array for upgrades
const upgrades = [
    { name: "A", initialCost: 10, currentCost: 10, rate: 0.1, button: document.createElement("button"), count: 0 },
    { name: "B", initialCost: 100, currentCost: 100, rate: 2.0, button: document.createElement("button"), count: 0 },
    { name: "C", initialCost: 1000, currentCost: 1000, rate: 50.0, button: document.createElement("button"), count: 0 },
];
...
function updateUpgradeCounts() {
    const counts = upgrades.map(upgrade => `${upgrade.name}: ${upgrade.count}`).join(", ");
